"""
Futures Trading - Box & Wedge Strategy
Backtesting interface for futures trading (ES/NQ) with Christian Carion's Box & Wedge methodology
"""
...
from utils.momentum import backtest_momentum_strategy
...
# Page configuration
st.set_page_config(
    page_title="Futures Trading - Box & Wedge",
    page_icon="📊",
    layout="wide"
)

st.title("📊 Futures Trading - Box & Wedge Strategy")
st.markdown("Backtest Christian Carion's Box & Wedge methodology on ES/NQ futures")

# Sidebar configuration
st.sidebar.header("Strategy Configuration")

# Strategy selection (Box & Wedge is primary, but other strategies available)
strategy_type = st.sidebar.selectbox(
    "Strategy Type",
    ["box-wedge", "buy-the-dip", "momentum"],
...
FUTURES_CONTRACTS = {
    ...
    "YM (Dow Jones E-mini)": "YM=F",
    "RTY (Russell 2000 E-mini)": "RTY=F"
}

# ETF proxies for futures (easier to backtest with more data)
FUTURES_PROXIES = {
    "SPY (S&P 500 ETF)": "SPY",
...
# Combine all options
all_options = {**FUTURES_CONTRACTS, **FUTURES_PROXIES}

# Default to ES and NQ
default_selections = ["ES (S&P 500 E-mini)", "NQ (Nasdaq 100 E-mini)"]

selected_names = st.sidebar.multiselect(
    "Select Futures/Proxies",
    options=list(all_options.keys()),
    default=default_selections,
    help="Select futures contracts or ETF proxies to backtest"
)

# Convert selected names to symbols
selected_symbols = [all_options[name] for name in selected_names]

# No global benchmarks for futures page
...
# Frequency/interval selection (focus on intraday for Box & Wedge)
interval = st.sidebar.selectbox(
    "Data Frequency",
    ["5m", "15m", "1h", "1d"],
    index=0,
    help="Time interval for price data (2m/5m recommended for Box & Wedge entries)"
)
...
if strategy_type == "box-wedge":
    # Box & Wedge specific parameters
    st.sidebar.markdown("**Box Detection**")
    contraction_threshold = st.sidebar.slider(
        "Contraction Threshold",
        min_value=0.5,
        max_value=0.9,
        value=0.7,
        step=0.05,
        help="Box contraction threshold (0.7 = 70% of average range)"
    )
    
    box_lookback = st.sidebar.number_input(
        "Box Lookback Periods",
        min_value=50,
        max_value=200,
        value=100,
        help="Periods to look back for box identification"
    )
    
    st.sidebar.markdown("**Wedge Detection**")
    wedge_lookback = st.sidebar.number_input(
        "Wedge Lookback Periods",
        min_value=10,
        max_value=50,
        value=20,
        help="Periods to look back for wedge within box"
    )
    ...
    risk_per_trade = st.sidebar.slider(
        ...
        value=1.0,
        step=0.1,
        help="Percentage of capital to risk per trade (Christian's 'All-In' concept)"
    )
    
    st.sidebar.markdown("**Scale Out Targets**")
    scale_out_1_5r = st.sidebar.slider(
        "1.5R Target (%)",
        min_value=0.0,
        max_value=100.0,
        value=50.0,
        step=5.0,
        help="Percentage of position to exit at 1.5R"
    )
    
    scale_out_3r = st.sidebar.slider(
        "3R Target (%)",
        min_value=0.0,
        max_value=100.0,
        value=25.0,
        step=5.0,
        help="Percentage of position to exit at 3R"
    )
    
    runner_pct = st.sidebar.slider(
        "Runner (%)",
        min_value=0.0,
        max_value=100.0,
        value=25.0,
        step=5.0,
        help="Percentage of position to keep as runner"
    )
...
if st.button("🚀 Run Backtest", type="primary", use_container_width=True):
    ...
    with st.spinner("Running backtest... This may take a moment."):
        try:
            ...
            if strategy_type == "box-wedge":
                results = backtest_box_wedge_strategy(
                    ...
                    scale_out_1_5r_pct=scale_out_1_5r,
                    scale_out_3r_pct=scale_out_3r,
